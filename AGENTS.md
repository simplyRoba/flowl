...
Guidance for coding agents working in `flowl` (Rust backend + SvelteKit frontend).

## Personas
- Developer: maintains the Rust service, Docker artifacts, and integration behavior.
- End user: runs `flowl` locally/self-hosted and accesses API/UI directly.
...
- Single integration test file: `cargo test --test plants`.
- Single test in an integration file: `cargo test --test plants create_with_defaults`.
- Exact single Rust test match: `cargo test create_with_defaults -- --exact`.
- Single UI test file: `npm run test --prefix ui -- ui/src/lib/stores/plants.test.ts`.
- Single UI test by name: `npm run test --prefix ui -- ui/src/lib/stores/plants.test.ts -t "sets plants on successful load"`.
- Vitest watch mode for one file: `npm --prefix ui exec vitest ui/src/lib/stores/plants.test.ts --watch`.
...
- File names: Svelte components use `PascalCase.svelte`; TS modules/stores/utils use lower-case or feature names like `plants.ts` and `thumbUrl.ts`.
...
- Backend: Axum + SQLx + SQLite + MQTT integration.
