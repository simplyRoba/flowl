...
| B8 | ~~**`Option<Option<T>>` complexity in UpdatePlant**~~ | 4 | 2 | ❌ | ~~Not a real issue. `Option<Option<T>>` with `#[serde(default)]` is the idiomatic Rust/serde pattern for PATCH semantics (absent vs explicit-null vs value). No crate does it meaningfully better.~~ |
