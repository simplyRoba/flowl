<script lang="ts">
  ...
  function statusSuffix(nd: string | null): string | null {
    ...
    const diffDays = Math.round(
      (dueStart.getTime() - start.getTime()) / 86400000,
    );
    if (diffDays === 0) return $translations.status.today;
    if (diffDays === 1) return $translations.status.inOneDay;
    if (diffDays > 1)
      return $translations.status.inNDays.replace("{n}", String(diffDays));
    const overdueDays = Math.abs(diffDays);
    return overdueDays === 1
      ? $translations.status.oneDay
      : $translations.status.nDays.replace("{n}", String(overdueDays));
  }
