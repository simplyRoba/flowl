...
`GET /auth/login` validates/bounds `return_to`, creates a private pre-auth `tower-sessions` session, and generates authorization URL values through `openidconnect`: random CSRF state, nonce, and `PkceCodeChallenge::new_random_sha256` plus verifier. Reuse the established transaction model: a process-local locked map keyed by state stores nonce, PKCE verifier, validated target, and absolute five-minute expiry, while the private tower session stores only that state binding.

Define `MAX_PENDING_LOGIN_TRANSACTIONS: usize = 1_024` as a hard process-wide registry maximum. While holding the registry lock, creation first removes every transaction with `expires_at <= now`, then checks capacity. If 1,024 valid entries remain, it leaves every active entry—including any prior transaction belonging to the initiating session—unchanged, does not insert or bind the new transaction, and returns the existing generic `/login?error=provider_unavailable&return_to=<safe-target>` path without redirecting to the OIDC provider. Only below capacity may a fresh login remove that session's prior transaction and insert/bind the replacement. Consuming or expiring an entry immediately makes capacity available to a later login.
