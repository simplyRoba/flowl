...
A `plants` table SHALL store plant entities with the following columns: `id` (integer primary key), `name` (text, required), `species` (text, optional), `icon` (text, default `🪴`), `photo_path` (text, nullable), `location_id` (integer, optional, foreign key to locations), `watering_interval_days` (integer, default 7), `last_watered` (text, nullable, ISO 8601), `light_needs` (text, default `indirect`), `notes` (text, optional), `created_at` (text, ISO 8601), `updated_at` (text, ISO 8601).
